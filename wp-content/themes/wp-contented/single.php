								...
								<div class="title-wrap">
									<div class="wrap title-wrap-inner">
										...
										<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
											<div class="date">
												<?php _e('By ','wpcontented'); ?><?php the_author_posts_link(); ?><?php printf( __( ' / <span>%2$s</span> / ', 'wpcontented' ), get_the_time('Y-m-j'), get_the_time(get_option('date_format'))); ?><?php $category_list = get_the_category_list( __( ', ', 'wpcontented' ) ); printf( __('%s', 'wpcontented'), $category_list); ?>
											</div>
				...
				<div class="wrap cf post-content-single">
					<article id="post-<?php the_ID(); ?>" <?php post_class('cf'); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
		               ...
		               <div class="next-prev-post">
			                  <div class="prev">
			                    <?php previous_post_link('<span>'. __('PREVIOUS POST','wpcontented') . '</span> &larr; %link'); ?>
			                  </div>
			                  <div class="next">
			                  <?php next_post_link('<span>'. __('NEXT POST','wpcontented') . '</span> %link &rarr;'); ?>
			                  </div>
			                  <div class="clear"></div>
		                </div> <!-- next-prev-post -->
		                ...
		                <div class="after-content-area <?php echo esc_attr( $widget_class ); ?>">
			                <footer class="article-footer <?php echo esc_attr( $author_class ); ?>">
			                  <h3 class="section-title"><?php _e('About Author ','wpcontented'); ?></h3>
			                  <div class="avatar">
		                    ...
		                    <?php if (!empty($related)): ?>
			                    <div class="related posts <?php echo esc_attr( $related_class ); ?>">
				                    <h3 class="section-title"><?php _e('Related Posts ','wpcontented'); ?></h3>
				                    <ul> 
						...
						<?php else : ?>
							...
							<article id="post-not-found" class="hentry cf">
									<header class="article-header">
										<h1><?php _e( 'Oops, Post Not Found!', 'wpcontented' ); ?></h1>
										<p><?php _e( 'Uh Oh. Something is missing. Try double checking things.', 'wpcontented' ); ?></p>
									</header>
							</article>
