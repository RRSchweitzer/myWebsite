...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
    function wpcontented_render_title() {
		?>
		<title><?php wp_title( '|', true, 'right' ); ?></title>
		<?php
	}
    add_action( 'wp_head', 'wpcontented_render_title' );
    
    add_filter( 'wp_title', 'wpcontented_rw_title', 10, 3 );
	function wpcontented_rw_title( $title, $sep, $seplocation ) {
	  global $page, $paged;
	  ...
	  if ( $paged >= 2 || $page >= 2 ):
	    $title .= " {$sep} " . sprintf( __( 'Page %s', 'wpcontented' ), max( $paged, $page ) );
	  endif;
...
function wpcontented_related_posts() {
	...
	if($tags) {
		...
		if($related_posts) {
			...
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'wpcontented' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_postdata();
	echo '</ul>';
...
function wpcontented_excerpt_more($more) {
	...
	return '...  <a class="excerpt-read-more" href="'. esc_url(get_permalink($post->ID),'wpcontented') . '" title="'. __( 'Read ', 'wpcontented' ) . get_the_title($post->ID).'">'. __( '[read more]', 'wpcontented' ) .'</a>';
}
	...
	register_field_group(array (
		...
		'fields' => array (
			array (
				...
				'label' => __('Embed Video','wpcontented'),
	...
	register_field_group(array (
		...
		'fields' => array (
			array (
				...
				'label' => __('Quote Content','wpcontented'),
	...
	register_field_group(array (
		...
		'fields' => array (
			array (
				...
				'label' => __('Upload Audio File Here','wpcontented'),
	...
	register_field_group(array (
		...
		'fields' => array (
			array (
				...
				'label' => __('Insert Short Status Here','wpcontented'),
	...
	register_field_group(array (
		...
		'fields' => array (
			array (
				...
				'label' => __('Insert Chat Conversation here','wpcontented'),
