...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'wpcontented-install-plugins';
         ...
         * does the wpcontented_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'installing'                     => __( 'Installing Plugin: %s', 'wpcontented' ),
                'oops'                           => __( 'Something went wrong.', 'wpcontented' ),
                ...
                'dashboard'                      => __( 'Return to the dashboard', 'wpcontented' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'wpcontented' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'wpcontented' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'wpcontented' ),
                'dismiss'                        => __( 'Dismiss this notice', 'wpcontented' ),
            );
            ...
            do_action_ref_array( 'wpcontented_init', array( $this ) );
        ...
        public function init() {
            ...
            do_action( 'wpcontented_register' );
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'wpcontented_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            if ( isset( $_POST['action'] ) && 'wpcontented-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="wpcontented wrap">
                ...
                <form id="wpcontented-plugins" action="" method="post">
                    <input type="hidden" name="wpcontented-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['wpcontented-install'] ) && 'install-plugin' == $_GET['wpcontented-install'] ) ) {
                check_admin_referer( 'wpcontented-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'wpcontented-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'wpcontented-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'wpcontented' ) . '">' . __( 'Return to the Dashboard', 'wpcontented' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['wpcontented-activate'] ) && 'activate-plugin' == $_GET['wpcontented-activate'] ) ) {
                check_admin_referer( 'wpcontented-activate', 'wpcontented-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'wpcontented' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'wpcontented' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'wpcontented' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'wpcontented_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'wpcontented-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'wpcontented', 'wpcontented', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'wpcontented', 'wpcontented', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'wpcontented' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['wpcontented-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'wpcontented_dismissed_notice', 1 );
            }
         ...
         */
        protected function is_wpcontented_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'wpcontented_dismissed_notice' );
    ...
    $wpcontented = TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'wpcontented' ) ) {
    /**
     ...
     */
    function wpcontented( $plugins, $config = array() ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'wpcontented' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'wpcontented' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'wpcontented' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'wpcontented' ) : __( 'Recommended', 'wpcontented' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'wpcontented' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'wpcontented' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'wpcontented' ) . ' %2$s">' . __( 'Install', 'wpcontented' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'wpcontented' ) . ' %2$s">' . __( 'Activate', 'wpcontented' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'wpcontented-activate'       => 'activate-plugin',
                                    'wpcontented-activate-nonce' => urlencode( wp_create_nonce( 'wpcontented-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'wpcontented' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'wpcontented' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'wpcontented' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'wpcontented' ),
                'source' => __( 'Source', 'wpcontented' ),
                'type'   => __( 'Type', 'wpcontented' ),
                'status' => __( 'Status', 'wpcontented' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'wpcontented-bulk-install'  => __( 'Install', 'wpcontented' ),
                'wpcontented-bulk-activate' => __( 'Activate', 'wpcontented' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'wpcontented-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => urlencode( TGM_Plugin_Activation::$instance->menu ),
                            'wpcontented-action'  => 'install-selected',
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'wpcontented-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'wpcontented-empty';
                    $i++;
                }
                ...
                echo '<div class="wpcontented wrap">';
            ...
            if ( 'wpcontented-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'wpcontented' ), $imploded );
                }
...
add_action( 'admin_init', 'wpcontented_load_bulk_installer' );
if ( ! function_exists( 'wpcontented_load_bulk_installer' ) ) {
    function wpcontented_load_bulk_installer() {
    /**
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function run( $options ) {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'wpcontented' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'wpcontented' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'wpcontented' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'wpcontented' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'wpcontented' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'wpcontented' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'wpcontented' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'wpcontented' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'wpcontented' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'wpcontented' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'wpcontented' ) . '</span><span class="hidden">' . __( 'Hide Details', 'wpcontented' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'wpcontented' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'wpcontented' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'wpcontented' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'wpcontented' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'wpcontented' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'wpcontented' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'wpcontented' ) . '</span><span class="hidden">' . __( 'Hide Details', 'wpcontented' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'wpcontented' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'wpcontented' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'wpcontented' ) . '">' . __( 'Return to the Dashboard', 'wpcontented' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
